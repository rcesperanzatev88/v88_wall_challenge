<?php
    class Message extends CI_Model{
        ...
        public function fetch_messages(){
            $result = array("status" => "true", "data" => "", "message" => "");
            ...
            $query = "SELECT messages.id, CONCAT(users.first_name, ' ' , users.last_name) as creator, message , DATE_FORMAT(messages.created_at, '%M %d %Y %h %i %p') as created_at, users.id as message_user_id,
                JSON_ARRAYAGG(
                    CASE WHEN comments.id IS NOT NULL THEN
                        JSON_OBJECT('id', comments.id, 'creator',  CONCAT(users2.first_name, ' ' , users2.last_name), 'message', comment, 'created_at', DATE_FORMAT(comments.created_at, '%M %d %Y %h %i %p'), 'message_user_id',  users2.id)
                    ELSE
                        JSON_ARRAY('')
        ...
        public function insert_message($message){
            ...
            $result["status"] == TRUE ? $result["message"] = "Message Saved" : $result["message"] = "Error in saving data";
        ...
        public function insert_comment($comment){
            ...
            $result["status"] == TRUE ? $result["message"] = "Comment Saved" : $result["message"] = "Error in saving data";
