...
class Message extends CI_Model{
    ...
    public function insert_message($message){
        ...
        $result["status"] == TRUE ? $result["message"] = 'Message Created Succesfully' : $result["message"] = 'Error in saving data';

        return $result;
    }

    #DOCU function to fetch all comments and messages using JSON
    public function fetch_messages(){
        $result = array("status" => "false", "data" => "", "message" => "");

        $query = "  SELECT messages.id, CONCAT(users.first_name, ' ', users.last_name) as creator, message, DATE_FORMAT(messages.created_at, '%M %d %Y %h %i %p') as created_at,
                    users.id as message_user_id,
                    JSON_ARRAYAGG( 
    ...
    #DOCU : function to insert new comment
    public function insert_comment($comment){
        ...
        $result["status"] == TRUE ? $result["message"] = 'Comment Created Succesfully' : $result["message"] = 'Error in saving data';

        return $result;
    }

    #DOCU function to fetch specific id at comments table
    public function fetch_comment($id){
        $result = array("status" => "false", "data" => "", "message" => "");

        $query = "  SELECT comments.id, CONCAT(users.first_name, ' ', users.last_name) as creator, comment, DATE_FORMAT(comments.created_at, '%M %d %Y %h %i %p') as created_at, users.id as message_user_id 
                    FROM comments 
      ...
      #DOCU function to fetch specific id at message table
     ...
     #DOCU function to delete message
     public function delete_message($id){
        $result = array("status" => "false", "data" => "", "message" => "");

        $query = " DELETE FROM messages WHERE messages.id = ? and user_id = ?";
